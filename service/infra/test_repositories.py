...
class TestInMemoryRepository(TestCase):
    def setUp(self):
        super().setUp()
        self.events = [
            self._create_event_mock({"category1", "category2", "default"}, {"all", "john"}),
            self._create_event_mock({"default"}, {"all", "no_one"}),
            self._create_event_mock({"category1", "cat2", "default"}, {"all"}),
            self._create_event_mock({"cat3", "cat2", "cat1", "default"}, {"no_one", "all"}),
            self._create_event_mock({"category1", "cat", "default"}, {"all"}),
            self._create_event_mock({"category", "default"}, {"john", "all"}),
            self._create_event_mock({"category1", "category2", "default"}, {"mike", "john", "all"}),
            self._create_event_mock({"category1", "category2", "category3", "default"}, {"mike", "all"}),
            self._create_event_mock({"category3", "default"}, {"jack", "mike", "all"}),
            self._create_event_mock({"without_default"}, {"nobody"}),
            self._create_event_mock({"category", "default"}, {"random person", "all"}),
            self._create_event_mock({"category", "category2", "default"}, {"all", "john"}),
            self._create_event_mock({"category1", "category2", "default"}, {"all"}),
        ]

        self.object_under_test = InMemoryRepository()
        self.object_under_test.events = self.events

    def _create_event_mock(self, categories, people):
        event = Mock(Event)

        category_matches = lambda category: (category in categories)
        person_matches = lambda person: (person in people)

        event.category_matches = category_matches
        event.person_matches = person_matches
    ...
    def test_should_return_less_than_ten_events_searching_by_category_given_less_matching(self):
        expected_result = [self.events[12], self.events[7], self.events[6], self.events[4], self.events[6],
